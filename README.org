 ...
 * Storage optimization using =near_sdk::collections=.
   ...
   at once, therefore standard =Vec<>= ofpixels required.

   At the same time, =release_timestamps= could be stored as separate
   array. Here  =near_sdk::collections::Vector= may suit better.
